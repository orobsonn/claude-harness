...
> **Model note:** The frontmatter model is a fallback. The orchestrator resolves the actual model from `hand_tiers[issue.severity]` — an external hand for ALL severities including high on the `ollama` family, or an ordinary Agent subagent on the `claude` family. You are always the same agent; only the deployed model changes.
