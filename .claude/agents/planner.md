...
- `model_strategy` is a frozen snapshot of model aliases (7 fixed eye roles incl. `plan-reviewer` + a hands map). **Emit the `hand_tiers` shape — it is the only valid shape** — pinning the **ACTIVE hand family's** weak→strong ladder verbatim, which decouples the hands from the always-Claude eyes. Read the active family first with `node .claude/shared/lib/hand-model-ladder.mjs show`: `claude` (the default) is `{ "low": "haiku", "medium": "sonnet", "high": "sonnet" }` — those rungs dispatch as ordinary Agent subagents, and the top rung escalates by reasoning effort (it resolves to the `executor-high`/`sniper-high` agent), never written into the plan — and `ollama` is `{ "low": "gemma4", "medium": "glm-5.2", "high": "kimi-k2.7-code" }`. The legacy Claude-only `tiers` map is **removed and rejected by validation**. **The ladder is an allowlist, not a suggestion:** `plan-write-gate` BLOCKS your Write unless `hand_tiers` equals the active family's ladder exactly — a flattened `sonnet/sonnet/sonnet`, a mixed ladder, or any other id is refused — so copy it verbatim. A task a cheap hand cannot do escalates (K=1), it does not get a hand tier of its own. Executor/sniper are NOT listed (model-variable: executor resolves from `hand_tiers[complexity ?? severity]`, sniper from `hand_tiers[issue.severity]`, at dispatch).
