...
- The command exits `0` only when the captured outcome is `DONE`; any other outcome exits non-zero → step 5 (sniper) / step 7 (escalation). The run-record it writes is the on-disk evidence (`captured: true`, real `exitCode` + `lockedTestExitCode` from the independent capture) — never the model's prose.
