...
- **Exit-code contract (the config-error escape — Trilho 5):**
  ...
  - `2` → **PRE-SPAWN config error or post-spawn critical exception** (no token, dirty baseline, gate not armed, missing/invalid test, diverged HEAD) — NOT a genuine run failure (`runLiveDispatch` RETURNS only on a genuine run and THROWS otherwise; the CLI emits `{ "configError": true, "reason": … }`). **Route exit 2 to the critical-exception path (step 7 below):** stamp `node .claude/hooks/mark.mjs hand-config-error --feature-id <feature-id> --task-id <task-id> --reason "<product-language reason>"` and SURFACE it — **NEVER a silent Claude fallback, NEVER a lock.** Without this the pipeline would deadlock the first time the token is absent.
- The run-record it writes is the on-disk evidence (`captured: true`, real `exitCode` + `lockedTestExitCode` from the independent capture) — never the model's prose. **The entry-gate authorizes a main-loop Claude `Agent(executor|sniper|test-author)` fallback ONLY when a stamped `escalation_fallback` ticket maps to an on-disk run-record whose `outcome` is `FAILED`** (a genuine run-and-fail). A config error writes no such record → the Claude escape is denied → the orchestrator MUST surface the critical exception, never fall back silently.
...
**Escalation-fallback ticket (Trilho 3 — deterministic precondition, survives compaction):** the Claude hand fallback is the **ONLY** legit main-loop `Agent(executor|sniper|test-author)` dispatch in v2 — the entry-gate hand-routing branch otherwise **DENIES** a main-loop `Agent(executor|sniper|test-author)` (in v2 those roles route through the spawn-hand path, not `Agent`). So **IMMEDIATELY BEFORE** dispatching the K=1 Claude `Agent(executor|sniper|test-author)` fallback, the orchestrator MUST stamp the ticket: run `node .claude/hooks/mark.mjs escalation-fallback --feature-id <feature-id> --task-id <task-id>`. **Without this ticket the entry-gate hand-routing branch DENIES the fallback** — the `Agent(executor|sniper|test-author)` dispatch is refused as an illegitimate inline-hand attempt. The ticket is the explicit precondition that authorizes the one legit escalation/transcription `Agent` fallback; stamp it, then dispatch. The SAME `escalation-fallback` ticket authorizes the test-author's K=1 transcription fallback (step 1b) — it is not a separate marker.

**The ticket alone is NOT enough (Trilho 5 — on-disk evidence belt):** the entry-gate now allows the Claude `Agent(executor|sniper|test-author)` fallback ONLY when the `escalation_fallback` ticket maps to an **on-disk run-record whose `outcome` is a genuine non-DONE run — `FAILED` (wrong work) OR `NOT_DONE` (empty diff)** — the non-forgeable evidence written by `runLiveDispatch`'s independent capture (real `exitCode` + `lockedTestExitCode`, never the model's prose). The record is **anchored to its `freeze_commit_sha`**: the gate cross-checks it against the current `HEAD`, so a STALE record from a prior run/freeze can never authorize a later, unfailed escalation (a positive freeze≠HEAD mismatch denies; an unreadable HEAD fails open). A stamped-but-recordless ticket (echo-forged, or a pre-spawn config error that produced no run) does NOT unlock the fallback. So a genuine cheap-hand run-and-fail (CLI exit `1` — `FAILED` or `NOT_DONE`) is the ONLY thing that authorizes the Claude escape; a config error (CLI exit `2`) routes to the critical exception instead.
...
**Hand config-error → critical exception (NOT a K=1 escalation):** when a cheap-hand dispatch exits `2` (pre-spawn config error: no Ollama token, dirty baseline relative to the freeze-commit, gate not armed, missing/invalid frozen test, diverged HEAD), the hand never ran — there is no genuine failure to escalate. Stamp `node .claude/hooks/mark.mjs hand-config-error --feature-id <feature-id> --task-id <task-id> --reason "<product-language reason>"` and route it to THIS critical-exception path (INTERACTIVE: surface to the operator; HEADLESS: open-PR risk item). **Never** retry into a silent Claude fallback (the entry-gate denies it — no on-disk `FAILED` record exists) and **never** silently lock the pipeline. This is the deterministic escape that keeps a missing token from deadlocking delivery.
