...
**Composition with cross-family (when on):** when the `codex-adversary` module is active (see *Cross-family eyes* above), the **adversary checkpoint** in this fan-out is itself a sub-flow — the Claude `adversary` Agent PLUS the Codex attack via `cross-family.mjs`, merged under **policy B** (codex-only findings get their Claude refutation folded in before the sniper). It enters the fan-out as a **single member** (the slowest one); `compliance` and `security` run concurrently with the whole adversary checkpoint, and the join waits for the **merged** adversary findings before step 5. Cross-family **widens** the adversary member — it is not a separate sequential stage and not a fourth independent fan-out member (the Codex attack consumes the Claude adversary's issues, so it composes inside the checkpoint, not beside it).
