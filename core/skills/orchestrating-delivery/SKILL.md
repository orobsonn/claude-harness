...
**Hand config-error → critical exception (NOT a K=1 escalation):** when a cheap-hand dispatch exits `2` (pre-spawn config error: no Ollama token, dirty baseline relative to the freeze-commit, gate not armed, missing/invalid frozen test, diverged HEAD), the hand never ran — there is no genuine failure to escalate. Stamp `node .claude/hooks/mark.mjs hand-config-error --feature-id <feature-id> --task-id <task-id> --reason "<product-language reason>"` and route it to THIS critical-exception path (INTERACTIVE: surface to the operator; HEADLESS: open-PR risk item). **Never** retry into a silent Claude fallback (the entry-gate denies it — no on-disk `FAILED` record exists) and **never** silently lock the pipeline. This is the deterministic escape that keeps a missing token from deadlocking delivery. **The Ollama token is a setup precondition, not something to discover per task:** LOCALLY it lives in the global `~/.claude/.dev.vars` (the orchestrator never has to create a project `.dev.vars`); in **HEADLESS** the global file does not exist in the cloud, so the token MUST be provided as an env secret on the cloud routine — if it is absent, `spawn-hand.mjs` exits `2` and the run records the missing-token setup gap as an open-PR risk item.
