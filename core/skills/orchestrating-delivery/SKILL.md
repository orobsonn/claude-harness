...
**v1 tier mapping for executor escalation:** In v1, LOW and MEDIUM executors run on Ollama; HIGH resolves to Claude. Escalation steps: LOW → MEDIUM (Ollama). MEDIUM-tier failure escalates directly to the **Claude hand fallback** — `hand_tiers.high` becomes the escalation target only after the v2 flip (when the high executor moves to Ollama). In v1 a single medium-tier failure lands on Claude directly.
