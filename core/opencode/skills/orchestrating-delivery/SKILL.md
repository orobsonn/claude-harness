...
- **Open material finding:** escalate to the operator in pt-br with its product impact before advancing. This is a human product checkpoint, not a runtime count, persisted review status, or dispatch denial.
...
**Task failures:** report provider or transient failures with their product impact. Continue only under the orchestrator's explicit judgment; OpenCode stores no same-agent retry count or terminal retry status.
