...
2. Still call native `mark` `hand-finished` / `capture-verified` if the host did not stamp (belt) — a `mark` that returns `ok:false` because the record is not DONE always means the hand did not deliver, so **do not ship**. But **`ok:false` is not one failure class — the two causes of a non-DONE record have opposite outlets, and the cause decides, never the record's state (#558 #ac-1.1):**
   - **The hand ran and refused — `BLOCKED` / `NEEDS_CONTEXT` / `CONFIG_ERROR`** (in the Task read-back or in the record): this is **not** a transient failure and gets **no retry at all**. Do **not** re-dispatch the hand; route straight to **CRITICAL EXCEPTION** (INTERACTIVE: operator in pt-br product language; HEADLESS: open PR risk item). This is the same outlet the other two rules for this class already state, and this step now agrees with both: § Per-task steps step b (`NEEDS_CONTEXT` / `BLOCKED` → *"route to critical exception — do not retry"*) and § Escalation ladder, *"Hand CONFIG_ERROR → critical exception (NOT a K=1 escalation)"*. Nothing broke — the hand refused — so no extra attempt can repair it; re-dispatching only burns that role's budget and delays the escalation.
   ...
   - **Reading the cause:** a refusal is a **verdict** — there is a `BLOCKED` / `NEEDS_CONTEXT` / `CONFIG_ERROR` read-back to point at. Absence of any read-back (the Task errored, timed out, or returned nothing) is the transient branch. Never infer a refusal from the non-DONE record alone.
