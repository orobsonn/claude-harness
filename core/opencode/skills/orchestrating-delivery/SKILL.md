...
| 4 | On a useful optional-eye result → call native `second-eye-prepare` with only the role; it consumes the host-captured second-eye Task result plus canonical routing and the matching primary gate-state receipt. It **merges under policy B** using shared `merge-findings.mjs` + `merge-verdicts.mjs`: a single-family finding is **kept unless the other family refutes it — never majority voting**. If it returns `dispatch-refute`, dispatch that exact signed descriptor once; the host captures its real Task result. Then call `second-eye-finalize` with only its `adjudication_id`. Route adopted findings through normal remediation (plan-review finding → planner; adversary finding → sniper). The returned `result` is the host-held primary report: caller JSON cannot replace either eye's report or refutation, and the second eye creates no separate gate |
...
**Refute-pass budget:** batch every second-eye-only finding from one useful optional-eye report into exactly **1** extra dispatch of that role's primary evaluator. The governing counter is `refute_pass_attempt_count`, enforced per `refute_pass_id` against `REFUTE_PASS_BUDGET` by `loop-decide.mjs`; it is separate from `plan_review_count` and `primary_review_failure_streak`. Never retry this pass. If the dispatch fails, returns malformed output, or exceeds its budget, the host captures that terminal outcome; call `second-eye-finalize` with only the `adjudication_id`, record `fail_safe_adopted: true` (and the gate-state `last_refute_budget_event` when exhausted), and adopt every unresolved second-eye finding by default. The dispatch consumes an epoch receipt, but refute failures use `refute_pass_failure_count` and never credit or reset `primary_review_failure_streak`. A failed optional-eye dispatch also remains outside primary accounting.
...
- `second-eye-prepare` / `second-eye-finalize` — native policy-B coordinator. The first returns the sole budgeted primary refute descriptor when needed; the second folds its result through shared merges and returns planner/sniper remediation without changing the primary verdict.
...
- **Phase 3 scope.** In the final review the CAP unit is the whole final review (round count does not restart between findings) and there is no per-task tier step to spend — but sniper dispatch and `regate-pending` / `regate-passed` stamps still use the **owning task's** `task_id` from the plan surface map (the host arm requires it; never omit). Signal an open finding into the `shipper` brief by attacked surface; a stronger-hand climb may still use that owning task's executor one tier up.
