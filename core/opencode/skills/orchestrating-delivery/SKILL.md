...
2. Still call native `mark` `hand-finished` / `capture-verified` if the host did not stamp (belt) — if mark returns `ok:false` because record is not DONE, treat as hand failure and re-dispatch the hand, do not ship. The budget for that re-dispatch is the **same-agent K=3 defined below in § Escalation ladder** — 3 attempts of *this specific hand*, counted per role(/task), reset on success. It is **not** the planner's K=3 (per plan-review round) and **not** the adversary's CAP = 3 rounds (per-task review loop); rounds already spent by those counters do not consume this hand's attempts.
