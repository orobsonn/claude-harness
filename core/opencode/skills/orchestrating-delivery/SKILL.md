...
2. **CANONICAL PATH — written for you by `planner-recovery` at:**
   ...
   **Approved-plan revision (narrow exception):** after APPROVE, delivery normally continues on the next legal task. Keep ordinary repairs inside the current or pending task's `scope_paths`; do **not** dispatch planner for routing/model drift, pre-existing debt, generic hardening, or a rare hypothesis. In the **same session**, planner is allowed only when concrete run evidence — a red locked test/gate or executor/compliance/security output — proves an approved acceptance criterion, locked contract, or material safety condition cannot be met within the approved scopes. Give planner the exact evidence, bound plan, and spec. It makes the **minimum revision** preserving the feature, mode, acceptance criteria, locked decisions, and observable product behavior; then run `validate-plan` and one `plan-reviewer` with `Review kind: REVISION`. A changed product behavior or contract is the only operator decision. Never automatically re-plan from an unanchored concern.
...
4. Dispatch `plan-reviewer` (read-only) for **engineering soundness**, then attempt `plan-reviewer-family-2` only when `roles.plan-reviewer.secondEyeModel` is set → `APPROVE | REVISE`. For the first pass, label the Task brief `Review kind: INITIAL`; every brief MUST require evidence anchors. On REVISE: retain the current plan in orchestration context, hand the exact findings to `planner`, re-plan, re-run `validate-plan`, then label the next reviewer brief `Review kind: REVISION` and include those exact prior findings plus a concise structural delta listing every changed, added, or removed task, `scope_paths`, `locked_tests`, and judgment. A re-review verifies that remediation and its direct consequences; it does not repeat broad discovery or turn a repaired plan into a new hunt for theoretical test gaps. It may REVISE only when an earlier finding remains or the revision itself introduces a material contract/safety contradiction. Do not dispatch a writing hand while a material REVISE remains; this is orchestrator discipline, not a persisted OpenCode gate. Escalate to the operator only for an explicit product decision, never because a hidden runtime count was spent.
...
After the `plan-reviewer` returns **APPROVE**, the bound plan is the default delivery contract for this feature. Preserve the task DAG, locked tests, and approved product decisions; ordinary engineering remains in the legal task rail, not a new planning cycle.
...
- If the evidence is **outside every approved `scope_paths`** and does not make an approved AC, locked test, or normal supported flow impossible, record a concise open risk / follow-up issue and continue the approved delivery. Routing drift, pre-existing debt, generic hardening, and a rare hypothesis stay in this category.
- A planner revision is allowed in the **same session** only when concrete run evidence — a red locked test/gate or executor/compliance/security output — proves an acceptance criterion, locked contract, or material safety condition cannot be met inside the approved scopes. Pass the exact evidence, bound plan, and spec to planner. It makes the **minimum revision**: preserve the feature, mode, acceptance criteria, locked decisions, and observable product behavior; change only the tasks and paths indispensable to resolve that contradiction. Then run `validate-plan` and one reviewer with `Review kind: REVISION`, which checks that evidence, the delta, and its direct consequences. A new binding may invalidate task-scoped capture facts, so this is a contract exception — never a shortcut for a normal repair.
- A material security, privacy, auth, corruption, or irreversible-harm finding cannot be downgraded to an open risk. If resolving it would change delivered product behavior or contract, stop only for that operator decision. Otherwise repair or make the minimum revision; do not automatically re-plan from an unanchored concern.
...
A fix bigger than surgical scope (re-architecture) is **not** a sniper job. Keep any in-scope repair with the approved task's executor rail. An out-of-scope repair follows **Approved-plan continuity**: it is a pending-task handoff, open risk, or — only with concrete contract evidence — the minimum planner revision described there. Never hand-write `execution-plan.json`.
