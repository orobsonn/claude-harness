...
before choosing `locked_tests[].path`. If they establish a layout, follow it for
new tests; do not move existing tests or change that layout to apply this default.
If no test layout is established, put all new test files under one root `tests/`
directory, grouped by domain or feature, with fixtures there too. The first
task introducing tests must scope any runner configuration needed to discover
`tests/`. An empty repository does not cancel an explicit convention.
