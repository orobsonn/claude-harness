---
description: Optional second-eye plan-reviewer (fail-open). Dispatched only when routing resolves a second eye.
mode: subagent
model: xai/grok-4.5
temperature: 0.1
permission:
...
On REVISE, be precise — one planner pass should fix it. The `revise_nudge` is the sole authority on the revision budget.
