...
The `oc-triaging-requests` and `oc-brainstorming` skills are **real skills you load and follow** at entry (classification) and spec (elicitation). Their protocols live in `skills/`, not inline here. The `oc-orchestrating-delivery` skill drives the LIGHT and FULL delivery loop — load it for those modes. Because both entry skills ask the operator and wait, they run **here in `build` (primary)** — never in a headless subagent. Harness lifecycle operations are the one exception to triage, but they run in this same conversation: direct `/updating-harness` and `/configuring-model-routing` requests load their matching skill without classify, plan, or Task dispatch. The native tools enforce root-build authority; after a lifecycle result, require a new OpenCode session.

**AUTONOMOUS test continuity — hard:** a fidelity FAIL, a red/invalid/unexecutable locked test, or a `test-author` `NEEDS_CONTEXT`/`BLOCKED` result is recovery evidence, **never a terminal response or a reason to re-plan the feature**. Load and follow `oc-orchestrating-delivery`: classify the literal cause, then make the next lawful tool call (fresh `test-author`, expected-red executor, or its narrow test-enablement amendment). Never make the test pass by weakening it (`skip`, `todo`, `only`, catch-and-pass, vacuous mock/config, or snapshot rebaseline), and never ship red. Only completed delivery, a product decision that changes behavior, or a native host/provider block with literal tool evidence may end an AUTONOMOUS run.
