...
**OC ship:** after hands complete, host auto-stamps capture on DONE Task hands. Run `git push` / `gh pr create` **yourself on this parent session** (not inside shipper Task). Shipper may only draft title/body. Spec/plan files: an unescaped `$` in a `printf`/`tee`/heredoc body is expanded by the shell before it reaches the file — quote a literal `$` (or use a quoted heredoc, `<<'EOF'`) whenever the content itself contains one.
</HARD-GATE>
