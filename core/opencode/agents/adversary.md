---
description: Primary adversary eye (Grok 4.5) — enters VIRGIN and finds failure modes that make the implementation unviable. Read-only. Always dual with adversary-openai.
mode: subagent
model: xai/grok-4.5
temperature: 0.3
permission:
...
> **Model note:** `xai/grok-4.5` — primary dual-eye post. Always paired with `adversary-openai` (`openai/gpt-5.5`). Build dispatches both; merge is T8 runtime (policy B). Do not skip dual.
