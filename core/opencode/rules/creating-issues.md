...
### Defeito desarmado parqueado como fonte (handoff do `oc-orchestrating-delivery`)
...
- **Como o `submit-issue.mjs` estampa `harness:ready` sempre, achado de run não passa pelo submitter**: a issue é criada à mão com `gh issue create` sem `--label`, exatamente como o candidato a aprofundamento (linha 59)
- **A trava é mecânica, não de prompt**: o entry-gate NEGA `gh issue create`/`gh issue edit` (e `gh api .../labels`, e uma chamada ao `submit-issue.mjs`, que estampa `harness:ready` sem flag visível) que anexe qualquer label `harness:*` quando quem chama é um **subagente** (o `harvester` sempre é) **ou** uma sessão de routine (headless-local/cron) — os dois sinais, porque o dispatch vivo do Orca não seta nenhuma variável de ambiente: a autonomia dele viaja no prompt. Instrução em prompt é exatamente o mecanismo que falhou aqui — a run leu "crie toda issue com `harness:ready`" e obedeceu
