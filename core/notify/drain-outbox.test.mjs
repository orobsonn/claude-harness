/**
 * @description Frozen behavioral tests for `drainTelegramOutbox` on ./notify-telegram.mjs.
 *
 * SPLIT AND REHOMED by issue #807 from `core/vps/drain-outbox.test.mjs`. The original file mixed
 * two subjects: `drainTelegramOutbox` itself (live — `notify-telegram.mjs` moved to `core/notify/`)
 * and its wiring into the retired Cron A composition root (`core/vps/run-cron-a.mjs`). The four
 * `mainCronA` tests were deleted WITH the engine they drive; the 35 below drive only
 * `drainTelegramOutbox` and the real obs-outbox persistence, and are moved verbatim.
 *
...
import { readEvents, readMeta, advanceCursor, updateMeta } from "../shared/lib/obs-outbox.mjs";
