...
description: Every writer of meta.threadId (core/notify/notify-telegram.mjs, core/shared/lib/obs-outbox.mjs) must also stamp meta.chatId in the same write — Telegram message_thread_id is per-chat, not globally unique, so a threadId without a matching chatId breaks any cross-tenant/cross-chat gate.
metadata:
  type: project
---
...
- `retention-sweep-delete-stale-topics` (#178) found and fixed the SECOND of two `threadId` writers:
  ...
  recreation branch in `drainTelegramOutbox` (today `core/notify/notify-telegram.mjs`) persisted a
...
- **Invariant:** `meta.chatId` must always describe the chat where the meta's CURRENT `threadId`
  lives. The live write surface is `core/notify/notify-telegram.mjs` (`drainTelegramOutbox`'s
  self-heal recreation branch, which carries this invariant as an in-code comment) and
  `core/shared/lib/obs-outbox.mjs` (`updateMeta` — the SOLE writer of `threadId`/`status`), plus any
  future recreation path. Grep every `threadId` write site in BOTH before adding a new one, and
