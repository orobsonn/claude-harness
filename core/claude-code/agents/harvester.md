...
- **`CHANGELOG.md`** — **release-please owns it** whenever the project shows release-please wiring of ANY shape: `release-please-config.json` at the repo root, **or** `.release-please-manifest.json`, **or** a workflow under `.github/workflows/` that runs `release-please-action`. In every one of those shapes it derives the changelog from Conventional Commits. Check all three — release-please is commonly configured entirely through workflow inputs, with no root config file, and a detection narrowed to that one file reads such a project as "manual" and appends a `CHANGELOG.md` entry that release-please is about to write again. That double-write is also a guaranteed merge conflict between any two concurrent runs. When it owns the file, the harvester never writes, appends, or edits `CHANGELOG.md`. Projects **without** release-please keep the manual fallback (add an entry under `## [Unreleased]`, Keep a Changelog format, one line per user-facing impact) as described in `core/rules/releases.md`.
