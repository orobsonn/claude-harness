...
export function reviewAutomationGuide() {
  return [
    ...
    "   o comando de merge NÃO pode passar -R/--repo (nem --auto), NEM carregar",
    "   pipe (|) ou redirecionamento (>) depois do `gh pr merge`, no mesmo",
    "   comando — qualquer um deles responde: PR target is ambiguous; merge is",
    "   denied.",
    "   Rode dentro do checkout do repo alvo e passe só o número do PR; se",
    "   precisar da saída, rode o merge sozinho e leia o resultado depois.",
    "   Isso é feature, não obstáculo — e deve continuar assim: é o que mantém",
    "   o gate de CI inescapável. Mecanismo e saídas medidas em",
    "   core/orca/README.md, seção \"Revisão de PR + merge condicional\".",
    "",
    "──────────────────────────────────────────────────────────────",
    "",
  ].join("\n");
