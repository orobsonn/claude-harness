...
**The hand family (operator toggle).** HANDS run on the `claude` ladder (`haiku`/`sonnet`/`sonnet`+`xhigh`, the default) or the `ollama` one (`gemma4`/`glm-5.2`/`kimi-k2.7-code`), persisted per project in `.claude/hand-config/hands.json` and read at **plan-write time only** — a frozen plan keeps the ladder it was authored with. When the operator asks to switch, run `node .claude/shared/lib/hand-model-ladder.mjs use <family>` (never a Write to `.claude/hand-config/` — the plan-write gate denies it) and say it applies to the NEXT plan. **Full mechanism, tokens, escalation and failure messages → load `references/hand-family.md` on demand.**
...
```json
{
  ...
  "effort": "<OPTIONAL, DERIVED from the rung (claude high → xhigh); absent when the rung has none>",
...
- **Token hygiene (load-bearing):** the hand token lives ONLY in env / `.dev.vars` — NEVER in the descriptor, argv, brief, or any log. `runLiveDispatch` fail-closes if the token literal appears in the descriptor bytes. **Both families need one** and both run under the ephemeral `CLAUDE_CONFIG_DIR` (`references/hand-family.md` — why inheriting the operator's config opens `Bash` and writes outside the repo).
...
- **Auth resolution — the orchestrator NEVER pre-checks the token (load-bearing):** `spawn-hand.mjs` resolves the ACTIVE dispatch family's token itself (env → project `.dev.vars` → global `~/.claude/.dev.vars`). The orchestrator MUST NOT `cat`/`grep`/`echo` `.dev.vars` to "verify the token is there" (a command naming `.dev.vars` is denied by the baseline anyway, while the resolver's internal read succeeds), and MUST NOT raise a "token missing" exception on its own judgment. The token counts as missing ONLY when `spawn-hand.mjs` exits `2` with a token `reason`, which names the exact key to export. Keys, setup and the sandbox rationale → `references/hand-family.md`. **This does NOT relax the git-universe reconciliation pre-step — that is separate from the token and stays mandatory.**
...
**Tier mapping for executor escalation:** LOW → MEDIUM → HIGH within `hand_tiers`, then — on the `ollama` family only — the **Claude hand fallback** (the final target; Claude is reachable by a hand ONLY via this K=1 escalation ticket). On the `claude` family there is no rung above HIGH, so a HIGH-tier failure goes straight to the critical exception. Per-family ladders and rationale → `references/hand-family.md`.
