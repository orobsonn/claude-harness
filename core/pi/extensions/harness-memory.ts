...
export default function harnessMemory(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "When a new base must be incorporated, use reconcile on the global parent before the first final review, or for a shipping base conflict; never task resume or a writer. Do not review or harvest just to unlock a merge. Supply full expected_head/base_sha. Omit resolutions for a read-only preview; supply [] for a clean merge. For each memory conflict give one hash-bound literal patch; for each product conflict inside the reviewed plan give one ordered literal patch per complete Git conflict hunk in patches. Preserve both sides' valid behavior and knowledge. Product conflicts outside plan scope stop without mutation. After integration, inspect changes, test and revalidate final input before harvest/shipping; no old receipt is promoted.",
    ...
    parameters: Type.Object({ action: StringEnum(["read", "status", "update", "apply", "reconcile", "finalize"] as const), content: Type.Optional(Type.String()),
      ...
      resolutions: Type.Optional(Type.Array(Type.Object({ path: Type.String(), before_sha256: Type.String(),
        patch: Type.Optional(Type.Object({ old_text: Type.String(), new_text: Type.String() })),
        patches: Type.Optional(Type.Array(Type.Object({ old_text: Type.String(), new_text: Type.String() }))) }))),
    }),
