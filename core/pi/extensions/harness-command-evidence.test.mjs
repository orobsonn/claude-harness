...
function fixture(t) {
  ...
  return { root, hooks, context, raw, run };
}
...
test("short output stays inline, with no artifact or extra result", async (t) => {
  const f = fixture(t);
  const { native, final } = await f.run(0, { command: "node -e 'console.log(123)'" });
  assert.deepEqual(final, native);
  assert.equal(final.content[0].text, "123\n");
});
...
test("stream metadata is scoped to its exact session and call and consumed once", async (t) => {
  const f = fixture(t);
  ...
  const result = { toolName: "bash", toolCallId: "shared-id", input: { command: "synthetic" }, content: [{ type: "text", text: "Command exited with code 1" }], isError: true };
  assert.equal(await f.hooks.get("tool_result")(result, f.context("session-b")), undefined);
  assert.equal(await f.hooks.get("tool_result")({ ...result, toolCallId: "other-id" }, f.context("session-a")), undefined);
  ...
  assert.equal(await f.hooks.get("tool_result")(result, f.context("session-a")), undefined);
});
...
test("stdout cannot select a file to copy and native metadata cannot redirect to a symlink", async (t) => {
  ...
  const event = { toolName: "bash", toolCallId: "fake-output", input: {}, content: [{ type: "text", text: "Full output: " + source }], isError: true };
  assert.equal(await f.hooks.get("tool_result")(event, f.context()), undefined);
