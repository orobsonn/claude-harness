...
| `harness-memory` | Herança automática de diário para filhos, atualização de contexto fora da sessão, revisão final sem harvest concluído e persistido, e limpeza antes de revisões atuais/árvore limpa. O pai recebe memória durável limitada e um ponteiro para o próprio `shared_context.md`; só a ferramenta lê/grava o diário. |
