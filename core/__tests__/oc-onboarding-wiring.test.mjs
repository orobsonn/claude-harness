...
test("CC updating-harness detects existing shells and documents all runtimes as the default", () => {
  assert.match(ccUpdating, /\.opencode\/\.harness-version/, "must detect install-vs-update on the OC shell marker");
  assert.match(ccUpdating, /default[\s\S]*Use `all` unless the operator explicitly selects/i, "must document all four as the default while preserving an explicit selection");
});
...
test("OC updating-harness skill exists and its native tool pins the lifecycle CLI to a git release", () => {
  ...
  assert.match(ocLifecycleToolRuntime, /defaults to all runtimes unless the operator explicitly selects/i, "tool must document all runtimes as the default while preserving an explicit selection");
});
