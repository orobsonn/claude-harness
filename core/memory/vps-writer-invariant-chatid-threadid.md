...
description: Every writer of meta.threadId in core/vps must also stamp meta.chatId in the same write — Telegram message_thread_id is per-chat, not globally unique, so a threadId without a matching chatId breaks any cross-tenant/cross-chat gate.
metadata:
  type: project
---
...
- `retention-sweep-delete-stale-topics` (#178) found and fixed the SECOND of two `threadId` writers:
  `setupObservability` (`cron-a-dispatch.mjs`) stamped `chatId` from `result.chatId` correctly, but
  the drain's self-heal recreation branch in `drainTelegramOutbox` (`notify-telegram.mjs`) persisted a
...
- **Invariant:** `meta.chatId` must always describe the chat where the meta's CURRENT `threadId`
  lives. Grep every write site of `threadId` in `core/vps/` (`setupObservability`,
  `drainTelegramOutbox`'s self-heal branch, and any future recreation path) before adding a new one,
  and confirm each also stamps `chatId` from the SAME API result (`createForumTopic`'s
  `result.chatId`) — never from `config.notify.chatId`, which is `undefined` on the common
  `.dev.vars`-only deployment (see `vps-notify-telegram.md`).
