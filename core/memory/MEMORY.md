...
- [Independent PR-review phase](independent-pr-review-phase.md) — cron-review.mjs/run-cron-review.mjs replace the editable-PR-body verdict with a fresh stateDir artifact + fail-closed conjunction; phase is wired but INERT until spawnReviewSession is implemented (also fix breaker now-threading + idempotency first)
