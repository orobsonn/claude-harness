...
- [Independent PR-review phase](independent-pr-review-phase.md) — cron-review.mjs/run-cron-review.mjs replace the editable-PR-body verdict with a fresh stateDir artifact + fail-closed conjunction; spawnReviewSession is now LIVE (real claude -p actuator, Node derives the verdict), breaker + idempotency fixed; open risks before enabling cross-family auto-merge documented in the topic file
