...
export const ROLES = {
  adversary: {
    ...
    outputContract: [
      ...
      "OPEN every `description` with the arming axis and reproduction status (rules/unarmed-defects.md):",
      "`<ARMED|UNARMED> · REPRO: <reproduced|not-reproduced|traced|not-attempted> — <what fails and the",
      "concrete trigger sequence>`. You run under `--sandbox read-only`: you may execute non-mutating",
      "commands, so reproduce when you can and use `traced` (path proved statically, citing file:fn) only",
      "when no runtime is reachable — NEVER lower a severity because you could not execute. ARMED = the trigger",
      "is a consequence of the design at the product's INTENDED scale, not whether it fires in today's",
      "install. UNARMED is an affirmative claim needing the required coincidence, the cited reason",
      "intended-scale operation does not produce it, the quoted source of that intended scale, and a",
      "closing `REARM: <concrete observable>`; missing any part, or in doubt, write ARMED. Anything wrong",
      "100% of the times its path executes is scope, not rarity — never UNARMED.",
    ].join("\n"),
  },
  "plan-reviewer": {
  ...
  security: {
    ...
    outputContract: [
      ...
      "evidence, fix_hint}]}, and nothing after it. UNSAFE requires at least one ARMED high or medium",
      "issue — a parked UNARMED finding is reported at its honest severity and does NOT set UNSAFE.",
      "OPEN every `description` with `<ARMED|UNARMED> · REPRO: <reproduced|not-reproduced|traced|",
      "not-attempted> — …` (rules/unarmed-defects.md). You CAN run read-only audit commands, so anything",
      "you rank above low carries a real attempt; `not-attempted` is incomplete and never parkable.",
      "UNARMED needs the required coincidence, the cited reason intended scale does not produce it, the",
      "quoted source of that scale, and a closing `REARM: <concrete observable>`; in doubt write ARMED.",
