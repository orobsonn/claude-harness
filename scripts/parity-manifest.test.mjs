...
const OC_RUNTIME_ADDITION_PATHS = new Set([
  "lib/worktree-baseline.mjs",
  "plugin/lib/plan-path-feature.mjs",
]);
...
const OC_FROZEN_MODULES = {
  ...
  "plugin/lib/plan-hash.mjs": { current_path: "lib/plan-hash.mjs", verdict: "DELETE" },
  "plugin/lib/plan-path-session.mjs": { current_path: "plugin/lib/plan-path-session.mjs", verdict: "DELETE" },
  "plugin/lib/plan-write-decide.mjs": { current_path: "plugin/lib/plan-write-decide.mjs", verdict: "REWRITE" },
  "plugin/lib/planner-artifact.mjs": { current_path: "lib/planner-artifact.mjs", verdict: "DELETE" },
  "plugin/lib/planner-brief.mjs": { current_path: "plugin/lib/planner-brief.mjs", verdict: "DELETE" },
  "plugin/lib/planner-fallback-config.mjs": { current_path: "lib/planner-fallback-config.mjs", verdict: "DELETE" },
  "plugin/lib/planner-result.mjs": { current_path: "plugin/lib/planner-result.mjs", verdict: "DELETE" },
  "plugin/lib/planner-state.mjs": { current_path: "lib/planner-state.mjs", verdict: "DELETE" },
  ...
  "plugin/lib/plan-path-feature.mjs": { current_path: "plugin/lib/plan-path-feature.mjs", verdict: "KEEP" },
  ...
  "plugin/planner-recovery.ts": { current_path: "plugin/planner-recovery.ts", verdict: "DELETE" },
...
const OC_MODULE_NORMATIVE_METADATA = {
  ...
  "plugin/lib/ceremony-binding.mjs": { failure_policy: "no remaining caller after R10 simplification", tests: ["scripts/parity-manifest.test.mjs"] },
  ...
  "plugin/lib/dispatch-scope.mjs": { failure_policy: "known plan, identity, or scope violation denies; storage fault opens with diagnostic", tests: ["core/opencode/lib/dispatch-scope.test.mjs", "core/opencode/lib/stable-dispatch-scope.test.mjs"] },
  ...
  "plugin/lib/plan-decide.mjs": { failure_policy: "invalid plan denies; validator exception opens and logs", tests: ["core/opencode/plugin/lib/plan-decide.test.mjs"] },
  "plugin/lib/plan-hash.mjs": { failure_policy: "no remaining caller after stable plan consumers hash exact bytes locally", tests: ["scripts/parity-manifest.test.mjs"] },
  "plugin/lib/plan-path-session.mjs": { failure_policy: "no remaining caller after observation adopted stable feature paths", tests: ["scripts/parity-manifest.test.mjs"] },
  "plugin/lib/plan-write-decide.mjs": { failure_policy: "state, unauthorized stable-plan, and out-of-scope writes deny; incomplete unrelated context opens", tests: ["core/opencode/plugin/plan-write-gate.test.mjs"] },
  "plugin/lib/planner-artifact.mjs": { failure_policy: "no remaining caller after direct stable-plan reads", tests: ["scripts/parity-manifest.test.mjs"] },
  "plugin/lib/planner-brief.mjs": { failure_policy: "no remaining caller after direct planner dispatch", tests: ["scripts/parity-manifest.test.mjs"] },
  "plugin/lib/planner-fallback-config.mjs": { failure_policy: "no remaining caller or routing role", tests: ["scripts/parity-manifest.test.mjs"] },
  "plugin/lib/planner-result.mjs": { failure_policy: "no remaining caller after direct planner write and conversational review", tests: ["scripts/parity-manifest.test.mjs"] },
  "plugin/lib/planner-state.mjs": { failure_policy: "no remaining caller after removing the planner lifecycle state machine", tests: ["scripts/parity-manifest.test.mjs"] },
  ...
  "plugin/lib/session-state.mjs": { failure_policy: "same-session read or validation faults omit recovery context and never gate", tests: ["core/opencode/plugin/reinject-state.test.mjs"], cc_op: ["CC_OP:core/claude-code/hooks/reinject-state.mjs"] },
  ...
  "plugin/lib/plan-path-feature.mjs": { failure_policy: "malformed path returns null", tests: ["core/opencode/plugin/lib/plan-path-feature.test.mjs"], cc_op: ["CC_OP:core/claude-code/hooks/obs-plan-write.mjs"] },
  ...
  "plugin/plan-gate.ts": { failure_policy: "missing or invalid stable plan denies; internal validator failure opens and logs", tests: ["core/opencode/plugin/stable-plan-gate.test.mjs"] },
  ...
  "plugin/planner-recovery.ts": { failure_policy: "no remaining caller after direct planner write and conversational review", tests: ["scripts/parity-manifest.test.mjs"] },
...
const OC_MODULE_NORMATIVE_REASONS = {
  ...
  "plugin/lib/dispatch-scope.mjs": "Call-keyed records freeze one validated stable-plan task scope for its producer.",
  ...
  "plugin/lib/plan-decide.mjs": "Validation must precede stable-plan use.",
  "plugin/lib/plan-hash.mjs": "A standalone semantic-hash layer is unnecessary for the stable plan.",
  "plugin/lib/plan-path-session.mjs": "Session-scoped path parsing was replaced by the stable feature path.",
  "plugin/lib/plan-write-decide.mjs": "Pure decision centralizes factual write ownership without composition machinery.",
  "plugin/lib/planner-artifact.mjs": "The stable feature plan removes the intermediate artifact layer.",
  "plugin/lib/planner-brief.mjs": "Planner context is prose in the Task prompt, as in Claude Code.",
  "plugin/lib/planner-fallback-config.mjs": "Model fallback strategy stays in prose and routing, not a native fallback planner.",
  "plugin/lib/planner-result.mjs": "No provider-result parser is needed when the planner writes the stable plan directly.",
  "plugin/lib/planner-state.mjs": "Claude Code has no parallel planner lifecycle state engine.",
  "plugin/lib/regate-arm.mjs": "Unused OC-only auto-arm contradicted the explicit parent-side re-gate flow.",
  "plugin/lib/roles.mjs": "Keep only the factual owner/hand/eye taxonomy.",
  "plugin/lib/scope-runtime-identity.mjs": "Resolve exact runtime identity without a mutable global context.",
  "plugin/lib/session-state.mjs": "Reinject only same-session triage and stable-plan facts.",
  ...
  "plugin/lib/plan-path-feature.mjs": "Observation extracts only the stable feature identity from plan/spec paths.",
  ...
  "plugin/plan-gate.ts": "Validate the stable plan without binding, reseal, or prompt mutation.",
  "plugin/plan-write-gate.ts": "Authenticate the planner for stable-plan writes and preserve exact hand scope.",
  "plugin/planner-recovery.ts": "Claude Code has no separate planner recovery controller.",
...
const OC_RULE_SEMANTICS = {
  ...
  R6: {
    ...
    implementation_targets: ["plugin/entry-gate.ts", "plugin/lib/bash-decide.mjs", "lib/entry-decide.mjs", "lib/gate-state.mjs", "plugin/lib/is-delivery-command.mjs", "plugin/marker-authority.ts"],
  ...
  R7: {
    ...
    implementation_targets: ["plugin/entry-gate.ts", "plugin/lib/bash-decide.mjs", "lib/gate-state.mjs", "lib/hand-records.mjs", "lib/worktree-baseline.mjs", "plugin/lib/host-hand-capture.mjs", "plugin/lib/is-delivery-command.mjs", "plugin/marker-authority.ts", "plugin/obs-hand.ts"],
  ...
  R8: {
    ...
    implementation_targets: ["plugin/entry-gate.ts", "plugin/lib/bash-decide.mjs", "lib/dispatch-scope.mjs", "lib/gate-state.mjs", "lib/hand-records.mjs", "lib/worktree-baseline.mjs", "plugin/lib/host-hand-capture.mjs", "plugin/lib/is-delivery-command.mjs", "plugin/marker-authority.ts"],
  ...
  R9: {
    ...
    implementation_targets: ["plugin/entry-gate.ts", "lib/entry-decide.mjs", "lib/gate-state.mjs", "lib/roles.mjs", "lib/task-dispatch-identity.mjs"],
  ...
  R10: {
    ...
    implementation_targets: ["plugin/entry-gate.ts", "lib/entry-decide.mjs", "lib/gate-state.mjs", "lib/roles.mjs", "plugin/marker-authority.ts"],
    locked_evidence: ["no ceremony sidecar/HMAC/recovery coordinator"],
  },
  R11: {
    required_outcome: "stable plan and dispatch preserve model strategy and canonical task scope",
    classification: "align to CC",
    failure_policy: "malformed stable plan or dispatch denies; observation failures open",
    implementation_targets: ["lib/dispatch-scope.mjs", "plugin/lib/session-state.mjs", "lib/task-dispatch-identity.mjs", "plugin/obs-hand.ts", "plugin/plan-gate.ts"],
    locked_evidence: ["stable plan task scope survives compaction and is frozen into each dispatch"],
  },
  R12: {
  ...
  R14: {
    required_outcome: "only planner authors/revises the stable plan",
    classification: "align to CC",
    failure_policy: "non-planner stable-plan write denies",
    implementation_targets: ["plugin/lib/plan-write-decide.mjs", "plugin/plan-write-gate.ts"],
    locked_evidence: ["orchestrator/eye/hand cannot write the stable plan"],
  },
  R15: {
    required_outcome: "stable plan validates model strategy before downstream use",
    classification: "align to CC",
    failure_policy: "missing or invalid content denies; internal validator fault opens/logs",
    implementation_targets: ["lib/dispatch-scope.mjs", "plugin/lib/plan-decide.mjs", "plugin/plan-gate.ts"],
    locked_evidence: ["legacy tiers and missing/invalid hand_tiers are rejected before dispatch"],
  },
  A1: {
...
describe("parity-manifest", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        'const ok = await import("../lib/obs-emit.mjs");\n' +
          'const gone = await import("../lib/missing-runtime.mjs");\n',
      );
      const res = checkImportsResolve(tmp);
      assert.equal(res.ok, false);
      assert.deepEqual(res.unresolved, [
        { file: join("tools", "classify.ts"), specifier: "../lib/missing-runtime.mjs" },
      ]);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("t11-lib-closure: surviving source and vendored framework libs have no plugin/lib back-imports", () => {
    const closure = [
      "gate-state.mjs",
      "entry-decide.mjs",
      "dispatch-scope.mjs",
  ...
  it("t12-module-manifest: freezes all 44 original modules and explicit runtime additions with the exact normative row schema", () => {
    ...
    assert.equal(
      manifest.modules.filter(({ original_path }) => !OC_RUNTIME_ADDITION_PATHS.has(original_path)).length,
      OC_INVENTORY_PROVENANCE.frozen_module_count,
    );
