...
if (!["evidence", "product", "follow-up", "harvest"].includes(scenario)) throw Error("Usage: node scripts/pi-convergence-pressure.mjs evidence|product|follow-up|harvest");
...
const customTools = [
  tool("harness_reviews", "Read host-bound review/capture evidence", Type.Object({ action: Type.String(), task_id: Type.Optional(Type.String()) }), evidence),
...
const local = ["evidence", "product"].includes(scenario);
...
try {
  await session.prompt(`Este é um pressure test de decisão com modelo real e evidência host injetada, não uma run de produto. Você é ${["evidence", "product"].includes(scenario) ? "o pai LOCAL da task task-one, modo FULL, plano estável aprovado" : "o pai global em finalização"}. Todos os dados necessários à decisão estão abaixo; não execute etapas posteriores à decisão pedida. Se a decisão exigir dispatch, use subagent; caso contrário, explique e encerre.\nEvidência atual verificada: ${JSON.stringify(evidence)}\n${request}`);
