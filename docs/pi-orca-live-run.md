# Run live: Pi em uma worktree Git

1. Abra a worktree da issue. O Orca pode gerenciá-la, mas não é necessário para executar o harness.
...
4. Depois da aprovação do plano, o pai global usa `harness_tasks` para despachar tasks independentes em paralelo, observar seus jobs, integrar o SHA exato de cada resultado e liberar as dependentes. Cada task executa a pipeline nativa completa em processo e worktree próprios.
5. Guarde o transcript JSON e verifique que ele contém triagem, plano, recibos de task/integração, testes do HEAD agregado, revisão independente e evidência final.
...
Uma retomada da mesma sessão e worktree usa `node .pi/harness/pi-harness.mjs --harness-resume <session-id> "Continue o plano aprovado até o PR draft."`. O launcher valida identidade, selo da spec e estrutura do plano antes de abrir o arquivo exato da conversa existente; não permite combinar esse comando com outros seletores de sessão/diretório. O pai relê o recibo host-owned da aprovação atual e reconcilia os handles persistidos por `harness_tasks`, sem duplicar processos. Abortar uma observação ou encerrar o pai global não cancela uma task já iniciada. Sem prova de aprovação, o plano volta ao plan-reviewer, não ao começo da spec. Uma nova sessão avulsa e uma mudança de host não são comprovadas por esse comando.
